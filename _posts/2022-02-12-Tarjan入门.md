...
那为什么 $low$ 取不到 $min\{low[v]\mid(u,v)\in S\}$ ($S$ 表示回边集合)? 原因其实很简单，首先是要根据其定义，然后就考虑为什么这样设计。比如说当我们求割点的时候，割点可以通过回边到达一个 $dfn$ 更小的点，但是我们既然删掉了这个点那就不影响了。如果这是其子树正好有到达 $u$ 点的回边，如果随着其 $low$ 跳到那个更低的点，那么结果是有问题的，这是会把 $u$ 误判为非割点。
